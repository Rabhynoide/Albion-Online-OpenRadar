...
| GPS / Avalon Roads | working | static open-world graph from `cluster/world.json`, road discovery by play, 24h staleness confirmed from game data (see `docs/technical/AVALON_ROADS_GPS.md`). Per-instance road duration still unobservable. Optional shared roads database via self-hosted `cmd/hub` (Docker, SQLite, shared-secret auth) - radar backend relays/falls back automatically, see `cmd/hub/README.md`. |
